...
3. 加工任务只能通过统一入口受控并发；同一输出数据集同一时刻最多运行一个任务。
...
FastAPI 和 Scheduler 分进程运行，分别使用异步和同步 SQLAlchemy 会话。Scheduler 内部使用最多 4 个采集线程；加工线程数由 `PROCESSING_MAX_WORKERS` 控制，默认 3、允许 1 至 8。两个执行器不共用线程池。生命周期 advisory lock 保证只有一个 Scheduler；加工领取使用独立 advisory lock、总运行数上限和输出数据集互斥，避免 5 秒定时唤醒与完成回调补位发生重复领取。
...
4. 加工链路：依赖解析、受控并发队列、同数据集串行、正式表转换和原子发布。
