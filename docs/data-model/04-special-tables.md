...
market_type和rank_type来自请求上下文。THS没有hot_type，固定rank_type='FINAL'；DC保存“人气榜/飙升榜”。rank_time按Asia/Shanghai解析。`dc_hot(is_new=Y)` 的历史请求仍可能一次返回多个合法时点；原始层全部保留，加工层按 `rank_time` 分组，以最大组行数识别完整快照并选择其中最新一组，随后校验股票和名次分别唯一。不得直接按股票去重，也不得用不完整的最晚组覆盖正式榜单。
