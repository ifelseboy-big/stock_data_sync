...
当前开发基线已调整 `ths_board_moneyflow_daily` 的主键与 `ts_code` 可空性，并移除 `market_theme_member_daily` 到同日题材排行的外键。项目仍处于以最新结构重建测试数据库的阶段，本次不提供旧测试数据迁移逻辑；部署验证前必须清理并按当前 Alembic 基线重建数据库，不能在旧表结构上直接运行新加工代码。
