...
| `theme_index_daily` | `ths_daily` | `theme_index` | 只发布主题指数主表中的代码 |
| `theme_index_member` | `ths_member`，按主题完整获取 | `theme_index`、`stock` | 只发布 `is_new=Y` 当前快照，按主题完整替换 |
| `stock_hot_rank_daily` | `ths_hot`、`dc_hot`，`is_new=Y` | `stock`、`trade_calendar` | DC 原始资产保留全部时点，加工选择时间最新且行数完整的快照 |
...
| 加工阶段 | 从统一入口受控并发加工并发布 | 同一数据集串行；加工失败重试不再调用 Tushare |
| 交易日 22:35 | 创建 HOT 批次，调用 `ths_hot`、`dc_hot` | `dc_hot` 即使在 `is_new=Y` 下返回多个 `rank_time`，也完整封存；加工选择最新完整快照 |
...
历史回填先由本地 `trade_calendar` 枚举开市日，再按业务日期创建 BACKFILL 批次。不同数据集起始日期和历史保留窗口不同：固定起始日期之前记为 `UNSUPPORTED`；滚动窗口外的真实空响应记为 `EMPTY_VALID` 零行资产，使同批次下游可以得到明确的空发布，而不是持续重试。
...
| 热榜 | 按 `source`、`market_type`、`rank_type` 拆分；THS直接使用最终榜，DC从原始多时点结果选择最新完整快照 |
...
| 题材、概念和主题成员代码 | 必须存在于包含 L/D/P/G 的股票主表，否则先刷新主数据 |
