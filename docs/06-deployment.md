...
v0.1.26 新增 `20260721_0012`，为采集任务增加不阻断发布的数据质量警告字段。v0.1.34 新增 `20260722_0013`，只创建活动加工恢复和活动采集批次领取的两个部分索引，不改写业务数据。升级到 v0.1.34 必须显式迁移并先备份数据库：

```bash
sudo stock-data-sync upgrade \
  --version 0.1.34 \
  --migrate \
  --database-backup /用户指定的绝对目录/stock-data-sync-before-0.1.34.dump
```
...
v0.1.36 新增 `20260722_0014`：关闭批次记录 DatasetSpec 目录指纹，加工规划按水位每轮限量处理；同一业务 Job 的定时、人工和启动补偿统一互斥；升级遗留的调度 `RUNNING` 自动收口；DATE 加工任务允许不同业务日期并行。历史关闭批次不会被迁移直接标记完成，而会按最新日期优先的有界队列重新验证，确保跳版本升级和此前死锁未完成的任务不会漏掉；迁移不改写正式业务表。升级必须显式迁移并先备份数据库：

```bash
sudo stock-data-sync upgrade \
  --version 0.1.36 \
  --migrate \
  --database-backup /用户指定的绝对目录/stock-data-sync-before-0.1.36.dump
```
