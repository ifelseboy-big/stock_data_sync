...
pre_install_doctor() {
  ...
  [[ -n "$INSTALL_DIR" && "$INSTALL_DIR" == /* && "$INSTALL_DIR" != "/" ]] && \
    doctor_pass "安装目录由用户明确指定" || doctor_fail "安装目录无效"
  if [[ -d "$INSTALL_DIR" ]]; then
    shopt -s nullglob dotglob
    local existing=("$INSTALL_DIR"/*)
    shopt -u nullglob dotglob
    (( ${#existing[@]} == 0 )) && doctor_pass "安装目录为空" || doctor_fail "安装目录不是空目录"
  else
    doctor_pass "安装目录尚未使用"
  fi
  ...
  [[ "$available_kb" =~ ^[0-9]+$ && "$available_kb" -ge 5242880 ]] && \
    doctor_pass "安装磁盘至少有 5 GiB 可用空间" || doctor_fail "安装磁盘可用空间不足 5 GiB"
...
deploy_validate_absolute_path "安装目录" "$INSTALL_DIR" || exit 1
...
if [[ -n "$BACKUP_DIR" ]]; then
  ...
  [[ "$BACKUP_DIR" != "$INSTALL_DIR" && "$BACKUP_DIR" != "$INSTALL_DIR/"* ]] || fail "备份目录不能位于安装目录内部"
fi
...
install -d -o "$SERVICE_USER" -g "$SERVICE_GROUP" -m 0755 \
  "$INSTALL_DIR/app" "$INSTALL_DIR/data/postgres" "$INSTALL_DIR/data/raw" \
  "$INSTALL_DIR/logs/postgres" "$INSTALL_DIR/logs/server" "$INSTALL_DIR/logs/scheduler" "$INSTALL_DIR/logs/backup" \
  "$INSTALL_DIR/backups"
install -d -o root -g wheel -m 0755 \
  "$INSTALL_DIR/bin" "$INSTALL_DIR/config" "$INSTALL_DIR/config/launchd" \
  "$INSTALL_DIR/source" "$INSTALL_DIR/releases" "$INSTALL_DIR/.build"
if [[ -n "$BACKUP_DIR" ]]; then
  install -d -o "$SERVICE_USER" -g "$SERVICE_GROUP" -m 0750 "$BACKUP_DIR"
fi

git clone --quiet --mirror "$BOOTSTRAP_MIRROR" "$INSTALL_DIR/source/repository.git"
git --git-dir="$INSTALL_DIR/source/repository.git" remote set-url origin "$REPOSITORY"
chown -R root:wheel "$INSTALL_DIR/source/repository.git"
chmod -R go-w "$INSTALL_DIR/source/repository.git"
TAG="$(deploy_resolve_tag "$INSTALL_DIR/source/repository.git" "$VERSION")"
deploy_prepare_release \
  "$INSTALL_DIR" "$SERVICE_USER" "$SERVICE_GROUP" "$SERVICE_HOME" \
  "$INSTALL_DIR/source/repository.git" "$TAG" "$REPOSITORY"
...
umask 077
{
  ...
  printf '# =============================================================================\n# 安装与应用基础配置\n# =============================================================================\n\n'
  printf '# 用户首次指定的安装目录。[安装器维护，请勿修改]\n'; deploy_write_env_value INSTALL_DIR "$INSTALL_DIR"
  ...
  printf '\n# =============================================================================\n# 数据、容量与前端配置\n# =============================================================================\n\n'
  printf '# 不可变 Parquet 原始资产目录。[安装器维护，请勿修改]\n'; deploy_write_env_value RAW_DATA_DIR "$INSTALL_DIR/data/raw"
...
chown root:wheel "$INSTALL_DIR/config/app.env"
chmod 0600 "$INSTALL_DIR/config/app.env"
chmod +a "$SERVICE_USER allow read" "$INSTALL_DIR/config/app.env"

install -o root -g wheel -m 0755 "$SCRIPT_DIR/bootstrap/installed-stock-data-sync" "$INSTALL_DIR/bin/stock-data-sync"
install -o root -g wheel -m 0755 "$SCRIPT_DIR/bootstrap/run-service" "$INSTALL_DIR/bin/run-service"
...
deploy_switch_current "$INSTALL_DIR" "$RELEASE_DIR"
deploy_write_receipt "$SERVICE_USER" "$SERVICE_GROUP" "$SERVICE_HOME" "$INSTALL_DIR" "$REPOSITORY" "$APP_VERSION"
...
/usr/bin/sudo -u "$SERVICE_USER" -H "$POSTGRES_BIN_DIR/initdb" \
  --pgdata="$INSTALL_DIR/data/postgres" --encoding=UTF8 --username=stock_sync \
  --pwfile="$PASSWORD_FILE" --auth-local=scram-sha-256 --auth-host=scram-sha-256
rm -f -- "$PASSWORD_FILE"
PASSWORD_FILE=""

"$INSTALL_DIR/bin/stock-data-sync" install-services
"$INSTALL_DIR/bin/stock-data-sync" start postgres
"$INSTALL_DIR/bin/stock-data-sync" migrate

if (( START_AFTER_INSTALL == 1 )); then
  "$INSTALL_DIR/bin/stock-data-sync" start
  "$INSTALL_DIR/bin/stock-data-sync" doctor --phase post-install
else
  "$INSTALL_DIR/bin/stock-data-sync" doctor --phase installed-no-start
fi
